 ...
 * 用户管理控制曾类
 */
@Api(value = "user", tags = "该接口为用户接口，主要实现用户登录、注册和校验token")
@RestController
@Slf4j
@RequestMapping("/user")
public class UserController {
    ...
    @ApiOperation(value = "用户注册", notes = "该接口主要实现用户注册账号", tags = {"user"})
    ...
    @ApiOperation(value = "用户登录", notes = "该接口主要实现用户登录认证后才能进入系统", tags = {"user"})
    @GetMapping("/login")
    @ResponseBody
    public RestResult<UserLoginVo> userLogin(
        ...
        try {
            //生成token
            jwt = JjwtUtil.createJWT("kekecloud", "keke", JSON.toJSONString(saveUserBean));
        } catch (Exception e) {
            log.info("登录失败：{}", e);
    ...
    @ApiOperation(value = "检查用户登录信息", notes = "该接口主要检查用户登录信息，验证token的有效性", tags = {"user"})
