...
public class ChunkUploader extends Uploader {
    ...
    public List<Integer> getChunks(String md5){
        String path = PathUtil.getStaticPath() + getSaveFilePath() + FILE_SEPARATOR + md5;
    ...
    private List<UploadFile> doUploadChunk(String savePath, Iterator<String> iter) throws IOException, NotSameFileExpection {
        ...
        uploadFile.setFileType(fileType);
        uploadFile.setTimeStampName(timeStampName);
        //D:\IdeaProjects\bishe\keke-cloud\target\classes\static \keke_files\yyyyMMdd\id.type
        //文件存储绝对路径
        String saveFilePath = savePath + FILE_SEPARATOR + timeStampName + "." + fileType;
        //D:\IdeaProjects\bishe\keke-cloud\target\classes\static \keke_files\yyyyMMdd\id
        //文件存储绝对路径（无后缀）
        String saveChunkPath = PathUtil.getStaticPath() + savePath + FILE_SEPARATOR + timeStampName;
        //D:\IdeaProjects\bishe\keke-cloud\target\classes\static \keke_files\yyyyMMdd\id\x
        //文件流
        ...
        if(file.getParentFile().list().length==uploadFile.getTotalChunks()){
            ...
            uploadFile.setUrl(saveFilePath);
            //合并文件
            File real = createFile( mkdir(PathUtil.getStaticPath() + savePath )+ FILE_SEPARATOR + timeStampName + "." + fileType);
            //临时文件
            File temp = null;
            ...
            //如果是图片 进行图片压缩 用于前端略览
            if (FileUtil.isImageFile(uploadFile.getFileType())){
                File minFile = createFile( mkdir(PathUtil.getStaticPath() + savePath )+ FILE_SEPARATOR + timeStampName +"_min"+ "." + fileType);
                ImageOperation.thumbnailsImage(file, minFile, 300);
            }
            log.info("合并完成");
            uploadFile.setSuccess(1);
